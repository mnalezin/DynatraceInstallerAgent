...
# cnf-cli installation script adjusted for Windows
# use it to replace the original install.sh script

set -e

if ! command -v python > /dev/null; then
    echo "python executable not found - please install Python 3"
    exit 1
fi

if ! python -c 'import sys; sys.exit(sys.version_info < (3, 6))'; then
    echo "python executable is too old - Python 3.6 or higher is required"
    exit 1
fi
...
env/Scripts/pip install --user --upgrade pip setuptools wheel > /dev/null
env/Scripts/pip install aws_cloudformation_rpdk-*-py3-none-any.whl aws_cloudformation_rpdk_java_plugin-*-py3-none-any.whl > /dev/null
...
ABSPATH="$PWD/env/Scripts/cfn-cli"
...
if ! env/Scripts/cfn-cli --help | grep -q "CloudFormation"; then
    echo "Installation failed:"
    set +e
    env/Scripts/cfn-cli --help
    exit 1
fi
...
echo "source $PWD/env/Scripts/activate"
